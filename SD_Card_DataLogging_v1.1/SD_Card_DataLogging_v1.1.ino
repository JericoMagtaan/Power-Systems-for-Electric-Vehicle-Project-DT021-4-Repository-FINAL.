...
*******************************************************************************/
/*
 * This programme is used to implement an SD card data logging system for the Power 
 * and Instrumentation Arduino.
 
 * Four DS18B20 sensors are used to measure temperature for the ACC temperature,
...
float tempC, Temp01, Temp02, Temp03, Temp04;  // Variables storing measured ACC individual sensor values.
float ACCTemp, ACCVoltage, ACCCurrent; // Variables storing measured ACC Temp, ACC current and ACC voltage values.
float MotorVoltageFL, MotorCurrentFL;  // Variables storing measured front left motor voltage and current values.
float MotorVoltageFR, MotorCurrentFR;  // Variables storing measured front right motor voltage and current values.
float TSPower;      // Variable storing measured total tractive system power values.
float CarSpeed;     // Variable storing measured average wheel speed sensor values.
...
void setup()
{
  ...
  myFile.println("Sensor01(degC),Sensor02(degC),Sensor03(degC),Sensor04(degC),ACC Temperature(degC),ACC Voltage(V),ACC Current(A),FL Motor Voltage(V),FL Motor Current(A),FR Motor Voltage(V),FR Motor Current(A),TS Power(kW)");      
  myFile.close();
...
void loop()
{
  ...
  // ACC average temperature data logging:
  ACCTemp = (Temp01 + Temp02 + Temp03 + Temp04)/4;   
  myFile.print(ACCTemp);  // Store averaged ACC temperature into Excel file.
  myFile.print(",");

  /*********************** ACC Voltage Data Log ***********************/
  ACCVoltage++;           // ACC voltage dummy variable increment.
  delay (100);            // 0.1 second delay.
  if (ACCVoltage>100)     // If ACC voltage reaches max range of 100V, reset back to 0V.
  {
    ACCVoltage = 0;
  }
  myFile.print(ACCVoltage);   // Store dummy ACC voltage into Excel file.
  myFile.print(",");


  /*********************** ACC Current Data Log ***********************/
  ACCCurrent++;            // ACC current dummy variable increment.
  delay (100);             // 0.1 second delay.
  if (ACCCurrent>650)      // If ACC current reaches max range of 650A, reset back to 0A.
  {
    ACCCurrent = 0;
  }
  myFile.print(ACCCurrent);   // Store dummy ACC current into Excel file.
  myFile.print(",");
